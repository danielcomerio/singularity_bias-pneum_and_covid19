...
def main():
    ...
    output_dir_radiography = os.path.join(
        OUTPUT_DIR, "COVID-19 Radiography Database")
    build_output_dirs(output_dir_radiography)
    ...
    offset = split_dataset_and_copy_files(
        files, labels, output_dir_radiography, general_loader, offset)

    output_dir_actualmed = os.path.join(
        OUTPUT_DIR, "Actualmed-COVID-chestxray-dataset-master")
    build_output_dirs(output_dir_actualmed)
    print("Processing the dataset actualmed")
    files, labels = preprocess_actualmed_ds(covid_actualmed_path)
    offset = split_dataset_and_copy_files(
        files, labels, output_dir_actualmed, general_loader, offset)

    output_dir_chestxray = os.path.join(
        OUTPUT_DIR, "covid-chestxray-dataset-master")
    build_output_dirs(output_dir_chestxray)
    print("Processing the dataset covid_chestxray")
    files, labels = preprocess_covid_chestxray_dataset(covid_chestxray_path)
    offset = split_dataset_and_copy_files(
        files, labels, output_dir_chestxray, general_loader, offset)

    output_dir_figure1 = os.path.join(
        OUTPUT_DIR, "Figure1-COVID-chestxray-dataset-master")
    build_output_dirs(output_dir_figure1)
    print("Processing the dataset figure1")
    files, labels = preprocess_figure1_covid_dataset(figure1_covid_path)
    offset = split_dataset_and_copy_files(
        files, labels, output_dir_figure1, general_loader, offset)

    output_dir_rsna = os.path.join(
        OUTPUT_DIR, "rsna-pneumonia-detection-challenge")
    build_output_dirs(output_dir_rsna)
    print("Processing the dataset rnsa")
    files, labels = preprocess_rnsa_dataset(rnsa_path)
    offset = split_dataset_and_copy_files(
        files, labels, output_dir_rsna, dicom_loader, offset)
